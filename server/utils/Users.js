...
const UserUtils = {
    ...
    checkEmailExistance: async (email, user_id = null) => {
        ...
        if (user_id) {
            queryObject = { email, _id: { $ne: ObjectId(user_id) }, status: { $ne: 2 } }
        }
    ...
    register: async (data) => {
        try {
            let { device_id, os, fcm_token, username, email, password, country_code, phone, input_source, profile_pic } = data;
            if (input_source == "email") {
                let emailExistanceResponse = await UserUtils.checkEmailExistance(email)
                if (!emailExistanceResponse?.status) {
                    // email already and check for password
                    let userData = emailExistanceResponse?.data;
                    if (userData?.password != "") {
                        delete emailExistanceResponse.data
                        return emailExistanceResponse
                    }
                    // password empty and update user
                    let editObj = {
                        username,
                        profile_pic,
                    ...
                    if (result?.status) {
                        ...
                        let dIndex = userData?.device_info.findIndex((it) => it.device_id == device_id)
                        if (dIndex < 0) {
                ...
                let phoneExistanceResponse = await UserUtils.checkPhoneExistance(phone)
                if (!phoneExistanceResponse?.status) {
                    // phone already and check for password
                    let userData = phoneExistanceResponse?.data;
                    if (userData?.password != "") {
                        delete phoneExistanceResponse.data
                        return phoneExistanceResponse
                    }
                    // password empty and update user
                    let editObj = {
            ...
            let usernameExistanceResponse = await UserUtils.checkUsernameExistance(username)
            if (!usernameExistanceResponse?.status) {
                return usernameExistanceResponse;
            }
            let newObj = {
                username,
                email: input_source == "email" ? email : "",
                phone: input_source == "phone" ? phone : "",
                password: md5(password),
                country_code,
                profile_pic,
                device_info: [{
    ...
    login: async (data) => {
        try {
            let { device_id, os, fcm_token, email_phone, password } = data
            let queryObject = { $or: [{ email: email_phone }, { phone: email_phone }], status: { $ne: 2 } }
            let result = await getSingleData(Users, queryObject, '');
            if (!result.status) {
